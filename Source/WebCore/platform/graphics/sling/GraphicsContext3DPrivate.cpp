...
bool GraphicsContext3DPrivate::initialize()
{
    ...
    if (m_context->m_renderStyle == GraphicsContext3D::RenderOffscreen) {
        m_offScreenSurface = GLPlatformSurface::createOffScreenSurface(GLPlatformSurface::SupportAlpha);

        if (!m_offScreenSurface)
            return false;

        if (!m_offScreenContext->initialize(m_offScreenSurface.get(), GLContext::sharingContext()->platformContext()))
            return false;
...
void GraphicsContext3DPrivate::restoreResources()
{
    releaseResources();
...
bool GraphicsContext3DPrivate::prepareBuffer()
{
    if (!makeContextCurrent())
        return false;
...
void GraphicsContext3DPrivate::createGraphicsSurface()
{
    ...
    if (m_graphicsSurface)
        m_graphicsSurface->graphicsSurfacePrivate()->reset(m_size, flags, m_offScreenContext->handle());
    else {
        m_graphicsSurface = GraphicsSurface::create(m_size, flags, m_offScreenContext->handle());
        m_surfaceHandle = (!m_graphicsSurface) ? GraphicsSurfaceToken() : GraphicsSurfaceToken(m_graphicsSurface->exportToken());
    }
...
uint32_t GraphicsContext3DPrivate::copyToGraphicsSurface()
{
    ...
    // FBO bounded texture should be flipped before being used as a source texture.
    m_graphicsSurface->graphicsSurfacePrivate()->copyFromTexture(m_context->m_texture, GraphicsSurfacePrivate::DrawOption::Flip);
    makeContextCurrent();
